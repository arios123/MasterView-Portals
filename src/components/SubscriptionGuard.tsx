...
//     checkSubscription();
//   }, [isPasswordRecovery,user, currentWorkspace, authLoading, workspaceLoading]);
...
export function SubscriptionGuard({ children }: SubscriptionGuardProps) {
  ...
  // 🔐 HARD BYPASS DURING PASSWORD RECOVERY
  ...
  useEffect(() => {
    ...
    checkSubscription();
  }, [user, currentWorkspace, authLoading, workspaceLoading]);

  // Disable scrolling when subscription guard is active (body + main content area)
  useEffect(() => {
    if (!hasAccess && currentWorkspace && !checking && !authLoading && !workspaceLoading) {
      ...
      return () => {
        ...
        if (main) (main as HTMLElement).style.overflow = prevMainOverflow;
      };
    }
  }, [hasAccess, currentWorkspace, checking, authLoading, workspaceLoading]);
  ...
  if (!hasAccess) {
    return (
      <>
        {children}
        <div className="fixed inset-x-0 top-16 bottom-0 z-40 bg-background flex items-center justify-center p-4 overflow-hidden touch-none overscroll-none">
          <Card className="w-full max-w-md shadow-lg">
            <CardHeader className="text-center pb-6">
              <div className="mx-auto mb-4 w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center">
                <CreditCard className="w-8 h-8 text-slate-600" />
            ...
            <CardContent className="space-y-4">
              {isOwner ? (
                <>
                  ...
                  <Button
                    onClick={() =>
                      navigate(`/choose-plan?workspace_id=${currentWorkspace.id}`)
                    }
                    className="w-full"
                    size="lg"
                  >
                ...
                <div className="text-sm text-muted-foreground space-y-2">
                  <p>• This workspace needs an active subscription.</p>
                  <p>• Please contact the workspace owner.</p>
                </div>
              )}
            </CardContent>
          </Card>
